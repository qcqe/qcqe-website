...
import { PROD_CATALOG } from '../sites/yeslon/data/products-catalog.mjs';

const PROD = PROD_CATALOG;
...
function itemFeatures(item, cat, name, desc) {
  if (item.features?.length) return item.features;
  return prodFeatures(cat, name, desc);
}
function itemScenes(item, cat, name) {
  if (item.scenes?.length) return item.scenes;
  return prodScenes(cat, name);
}
function itemSpecs(item) {
  return item.specs?.length ? item.specs : [];
}

function prodPageBody(cat, item, pp, c, pfx) {
  const flags=itemFeatures(item,cat,item.n,(item.d||''));
  const scenes=itemScenes(item,cat,item.n);
...
<div class="flex flex-wrap gap-2 mt-4"><span class="px-3 py-1 bg-white/20 text-white text-sm rounded-full">${h(cat)}</span>${specs.length?`<span class="px-3 py-1 bg-white/10 text-primary-200 text-sm rounded-full">${specs.length} 项核心参数</span>`:''}</div>
</div></div>
...
${specs.length?`<section class="py-12 bg-gray-50"><div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
<h2 class="text-2xl font-bold mb-8 flex items-center gap-2">技术参数</h2>
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
<table class="w-full text-sm"><tbody>${specs.map((s,i)=>`<tr class="${i%2===0?'bg-white':'bg-gray-50'}"><td class="py-3 px-5 text-gray-600 font-medium w-1/3 border-r border-gray-100">${h(s.k)}</td><td class="py-3 px-5 text-gray-800">${h(s.v)}</td></tr>`).join('')}</tbody></table>
</div></div></section>`:''}

${item.models?.length?`<section class="py-12 bg-white"><div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
<h2 class="text-2xl font-bold mb-6">典型型号</h2>
<ul class="grid md:grid-cols-2 gap-2 text-sm text-slate-700">${item.models.map(m=>`<li class="flex gap-2 p-3 bg-slate-50 rounded-lg"><span class="text-primary-600">▸</span><span>${h(m)}</span></li>`).join('')}</ul>
</div></section>`:''}
...
function prodsPage(pp, c, sn, pfx, p) {
  ...
  const tableRows = cats.flatMap(cat=>cat.items.map(item=>{
    const specs=itemSpecs(item);
    const specHint=specs.length?specs.slice(0,2).map(s=>s.k+': '+s.v).join('；'):'';
    ...
    <td class="py-3 px-4 text-xs text-slate-500 hidden lg:table-cell">${h(specHint||'—')}</td>
    <td class="py-3 px-4 text-right"><a href="${pfx}/products/${slug(item.n)}" class="text-xs text-primary-800 font-medium no-underline">参数</a></td>
  </tr>`;
  })).join('');
...
<div class="flex items-center gap-3">${catMark(cat.cat)}<div><h2 class="text-xl font-bold text-slate-900">${h(cat.cat)}</h2>${cat.desc?`<p class="text-sm text-slate-500 mt-1 max-w-2xl">${h(cat.desc)}</p>`:''}</div><span class="text-xs text-slate-500 ml-2 shrink-0">${cat.items.length} 款</span></div>
<a href="${pfx}/products" class="text-xs text-slate-500 hover:text-primary-800 no-underline shrink-0">返回目录</a>
</div>
<div class="grid md:grid-cols-2 lg:grid-cols-3 gap-3">${cat.items.map(item=>{
  const specs=itemSpecs(item);
  const specList=specs.slice(0,4).map(s=>`<li class="text-[11px] text-slate-500"><span class="text-slate-600">${h(s.k)}</span> ${h(s.v)}</li>`).join('');
  return `<div class="card-panel p-4 flex flex-col">
<h3 class="font-semibold text-slate-900 text-sm"><a href="${pfx}/products/${slug(item.n)}" class="no-underline text-inherit hover:text-primary-800">${h(item.n)}</a></h3>
${item.m?'<p class="text-xs text-slate-500 mt-1 font-mono">'+h(item.m)+'</p>':''}
<p class="text-xs text-slate-600 mt-2 leading-relaxed line-clamp-2 flex-1">${h((item.d||item.desc||'').slice(0,120))}</p>
${specList?`<ul class="mt-3 space-y-1 border-t border-slate-100 pt-3">${specList}</ul>`:''}
<a href="${pfx}/products/${slug(item.n)}" class="text-xs text-primary-800 font-medium mt-3 no-underline self-start">完整参数 →</a>
</div>`;
}).join('')}
</div></div></section>`).join('');

  const bd=`${nav(pp,c,pfx+'/products',pfx)}
${pageHero('产品中心',c.name+' 产品目录 · 按业务线分类 · 含核心型号与技术参数')}
...
<th class="py-3 px-4 font-medium">产品名称</th><th class="py-3 px-4 font-medium">型号</th><th class="py-3 px-4 font-medium">分类</th><th class="py-3 px-4 font-medium hidden lg:table-cell">核心参数摘要</th><th class="py-3 px-4 font-medium text-right">详情</th>
...
var yeslonCats=PROD.yeslon||[];
for(var ci=0;ci<yeslonCats.length;ci++){var cat=yeslonCats[ci];var items=cat.items||[];for(var ii=0;ii<items.length;ii++){var item=items[ii];addItem(item.n,item.d||'',siteUrl+'/products/'+slug(item.n),'产品',cat.cat||'')}}
// Solutions
